...
import { useRetailAutomations, useToggleRetailAutomation, useUpdateRetailAutomation } from '../hooks/useRetailAutomations';
...
export default function RetailAutomationsPage() {
  const [editDialog, setEditDialog] = useState({ open: false, automation: null });
  const [editMessage, setEditMessage] = useState('');
  const { data, isLoading, error, refetch } = useRetailAutomations();
  const toggleAutomation = useToggleRetailAutomation();
  const updateAutomation = useUpdateRetailAutomation();
  ...
  if (isLoading) {
    return <LoadingBlock message="Loading automations..." />;
  }
  ...
  return (
    <div>
      <PageHeader
        title="Automations"
        description="Manage automated SMS triggers"
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {automations.length === 0 ? (
          <EmptyState
            ...
            description="Set up automated SMS triggers for your campaigns"
          />
        ) : (
