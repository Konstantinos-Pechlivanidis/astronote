...
import { useRetailContacts, useImportRetailContacts } from '../hooks/useRetailContacts';
...
export default function RetailContactsPage() {
  const [page, setPage] = useState(1);
  const [search, setSearch] = useState('');
  const [importDialogOpen, setImportDialogOpen] = useState(false);
  const { data, isLoading, error, refetch } = useRetailContacts({ page, limit: 20, search });
  const importContacts = useImportRetailContacts();
