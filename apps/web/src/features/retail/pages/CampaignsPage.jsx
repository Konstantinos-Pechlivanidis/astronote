...
import { useRetailCampaigns, useEnqueueRetailCampaign, useDeleteCampaign } from '../hooks/useRetailCampaigns';
...
function useDeleteRetailCampaign() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (id) => {
      const { data } = await axiosRetail.delete(`/campaigns/${id}`);
      return data;
    },
    onSuccess: () => {
...
export default function RetailCampaignsPage() {
  ...
  const { data, isLoading, error, refetch } = useRetailCampaigns({
    page,
    pageSize: 20,
    search,
    status: statusFilter || undefined,
  });
  const enqueueCampaign = useEnqueueRetailCampaign();
  const deleteCampaign = useDeleteRetailCampaign();
  ...
  if (isLoading) {
    return <LoadingBlock message="Loading campaigns..." />;
  }
  ...
  return (
    <div>
      <PageHeader
        ...
        action={() => navigate('/retail/campaigns/new')}
        actionLabel={
          <>
      ...
      <DataTable
        ...
        emptyAction={() => navigate('/retail/campaigns/new')}
