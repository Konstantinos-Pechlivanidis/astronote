...
export default function ShopifyListsPage() {
  ...
  if (isLoading) {
    return <LoadingBlock message="Loading Shopify segments..." />;
  }

  if (error) {
    return (
      <ErrorState
        title="Failed to load Shopify segments"
  ...
  return (
    <div>
      <PageHeader
        title="Shopify Lists & Segments"
        description="Predefined audience segments for targeting in Shopify"
      />
      ...
      {segments.length === 0 && (
        <EmptyState
          title="No Shopify segments available"
          description="System segments will be created automatically."
        />
      )}
    </div>
  );
