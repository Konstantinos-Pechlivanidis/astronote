...
async function appendTxnAndUpdate(
  ...
  delta,
  type,
  { reason, campaignId, messageId, meta, idempotencyKey } = {},
  tx = null,
) {
  const execute = async client => {
    // P0: Check idempotency if key provided
    if (idempotencyKey) {
      ...
      if (existing) {
        ...
        return { balance: wallet?.balance || 0, txn: existing };
      }
    }
...
export async function createCreditTransaction(
  ...
  const execute = async client => {
    ...
    logger.info(
      { shopId, type, amount: Math.abs(amount), reason, idempotencyKey: idempotencyKey ? `${idempotencyKey.substring(0, 8)}...` : null },
...
export async function reserveCredits(
  ...
  const { campaignId, expiresAt, meta, reservationKey } = options;

  const execute = async client => {
    // P0: Check idempotency if reservationKey provided
    if (reservationKey) {
