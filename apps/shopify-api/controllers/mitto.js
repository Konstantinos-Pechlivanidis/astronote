...
export async function deliveryReport(req, res) {
  try {
    ...
    // P0: Webhook replay protection - process each event with deduplication
    ...
    for (const ev of events) {
      ...
      // P0: Generate event ID for replay protection (messageId + status + timestamp bucket)
      const eventId = `${providerId}:${statusIn}:${Math.floor(doneAt.getTime() / 60000)}`; // Bucket by minute
      const eventHash = webhookReplay.generateEventHash(JSON.stringify(ev));

      // Check replay protection
      const existing = await webhookReplay.checkWebhookReplay('mitto', eventId, eventHash);
      if (existing) {
        logger.debug('Mitto DLR webhook already processed (replay)', {
          providerId,
          eventId,
          statusIn,
        });
