...
export async function enqueue(req, res, next) {
  try {
    ...
    if (idempotencyKey) {
      ...
      const existing = await idempotencyService.checkEnqueueRequest(storeId, id, idempotencyKey);
    ...
    if (idempotencyKey && result.ok) {
      try {
        ...
        await idempotencyService.recordEnqueueRequest(storeId, id, idempotencyKey, responseData);
      } catch (recordError) {
        // Don't fail the request if idempotency recording fails
        logger.warn('Failed to record enqueue request idempotency', {
          storeId,
          campaignId: id,
