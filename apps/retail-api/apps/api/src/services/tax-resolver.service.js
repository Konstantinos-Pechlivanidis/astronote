...
const logger = pino({ name: 'tax-resolver' });
...
const resolveTaxTreatment = ({
  ...
  if (!country) {
    logger.warn({ billingCountry, ipCountry }, 'Tax resolver missing country');
    return {
      mode: 'unknown',
      taxRate: null,
...
const resolveTaxRateForInvoice = ({ subtotal, tax }) => {
