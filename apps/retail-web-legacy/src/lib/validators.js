...
export const campaignSchema = z.object({
  ...
  // P0 FIX: Removed templateId - campaigns are independent from templates
  messageText: z
    .string()
    .min(1, 'Message text is required')
    .max(2000, 'Message text must be 2000 characters or less'),
  // BUG FIX: Accept empty string for "Any" option, transform to null
  // Empty string '' from select dropdown should be treated as null (no filter)
  filterGender: z
    .union([
...
export const profileUpdateSchema = z.object({
  company: z.string().max(160, 'Company name must be 160 characters or less').optional().or(z.literal('')).nullable(),
  senderName: z
    .string()
    .max(11, 'Sender name must be 11 characters or less')
    .regex(/^[a-zA-Z0-9]*$/, 'Sender name must contain only letters and numbers')
    .optional()
    .or(z.literal(''))
    .nullable(),
  timezone: z
    ...
    .refine(
      (val) => {
        ...
        return /^[A-Za-z_]+\/[A-Za-z_]+$/.test(val);
      },
      {
...
export const changePasswordSchema = passwordChangeSchema; // Alias for backward compatibility
