...
export async function GET(req: NextRequest, { params }: { params: { shortCode: string } }) {
  ...
  if (retail.location && debug) {
    return NextResponse.json({ type: 's', token, attempts, final: 'redirected', location: retail.location, base: RETAIL_API_BASE }, { status: 200 });
  }

  if (debug) {
    return NextResponse.json({ type: 's', token, attempts, final: 'not_found', base: RETAIL_API_BASE }, { status: 404 });
  }
