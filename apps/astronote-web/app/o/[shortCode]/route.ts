...
export async function GET(req: NextRequest, { params }: { params: { shortCode: string } }) {
  ...
  const retail = await resolveRedirect(token, RETAIL_API_BASE, 'o', 'retail');
  ...
  const shopify = await resolveRedirect(token, SHOPIFY_API_BASE, 'o', 'shopify');
  ...
  if (shopify.location && debug) {
    return NextResponse.json({ type: 'o', token, attempts, final: 'redirected', location: shopify.location }, { status: 200 });
  }
  ...
  fallback.searchParams.set('type', 'o');
